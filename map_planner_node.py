...
class MapPlannerNode(Node):
    ...
    def __init__(self, log_dir: str = None, log_timestamp: str = None):
        ...
        # ========== 并行执行：创建独立的 callback groups（不设置锁）==========
        self._data_group = MutuallyExclusiveCallbackGroup()      # gps_path, map_pose 共用
    ...
    def convert_gps_points_to_map_points(
        self,
        ...
        """
        将 GPS 航点列表转换为 map 坐标系下的航点列表

        Args:
            gps_points: GPS 航点列表，每个元素包含 'latitude' 和 'longitude'
    ...
    def gps_path_callback(self, msg: String):
        ...
        if trans_x is None:
            self.logger.warning('utm<-map transform not available, cannot convert GPS points')
            return

        self.nav_map_points = self.convert_gps_points_to_map_points(
            list(self.nav_gps_points), trans_x, trans_y, yaw
        )
    ...
    def generate_map_and_nav_points(self, nav_map_points: List[dict]) -> bool:
        ...
        self.draw_road_on_grid(grid, self.map_metadata, full_path)
        ...
        # 发送未膨胀的全局地图用于可视化初始化
        grid_msg = self.build_map_msg(self.map_data, self.map_metadata)
        ...
        if self.inflation_enabled:
            ...
            grid_msg = self.build_map_msg(self.inflated_map_data, self.map_metadata)
            if grid_msg is not None:
    ...
    def build_map_msg(self, map_data: Optional[np.ndarray], map_metadata: MapMetadata) -> Optional[OccupancyGrid]:
        ...
        grid_msg.info.resolution = float(map_metadata.resolution)
        grid_msg.info.width = int(map_metadata.width)
        grid_msg.info.height = int(map_metadata.height)
        ...
        grid_msg.data = map_data.ravel().tolist()
        return grid_msg
    ...
    def _planning_timer_callback(self):
        ...
        pose = self.latest_map_pose
        if pose is None:
            self.logger.debug('_planning_timer_callback: latest_map_pose is None')
            return
    ...
    def plan_once(self, local_costmap_stamp: Optional[Time] = None):
        ...
        pose = self.latest_map_pose
        if pose is None:
            self.logger.debug('plan_once: latest_map_pose is None')
            return
        pose_timestamp = pose.get('timestamp', 0)
        if current_nanos - pose_timestamp > self.map_pose_timeout * 1e9:
...
def run_map_planner_node(log_dir: str = None, log_timestamp: str = None, args=None):
    ...
    # 使用多线程执行器，支持三个线程组并行执行
    # 线程分配：
    #   线程1: _data_group (gps_path, map_pose)
