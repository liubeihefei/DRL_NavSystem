...
class Nav2GPSNode(Node):
    ...
    def start_nodes(self):
        ...
        if not is_test_mode:
            ...
            self.controller_node = ControllerNode(log_dir=self.log_dir, log_timestamp=self.start_timestamp)
...
def main(args=None):
    ...
    # ekf_fusion_node 初始化：等待初始数据收集并执行 tf_init
    nav_node.get_logger().info('Waiting for ekf_fusion_node initial sensor data collection...')
    init_executor = SingleThreadedExecutor()
    init_executor.add_node(nav_node.ekf_fusion_node)
    start_time = nav_node.ekf_fusion_node.get_clock().now().nanoseconds / 1e9
    init_timeout = 3.0

    while rclpy.ok():
        current_time = nav_node.ekf_fusion_node.get_clock().now().nanoseconds / 1e9
        if current_time - start_time > init_timeout:
            break
        init_executor.spin_once(timeout_sec=0.1)

    nav_node.get_logger().info('Initial data collection complete. Running tf_init...')
    if not nav_node.ekf_fusion_node.tf_init():
        nav_node.get_logger().error('ekf_fusion_node tf_init failed! Node will continue but may not function correctly.')

    init_executor.remove_node(nav_node.ekf_fusion_node)
